...
/** ----------------------------------------------------------------------------------------------------------------------- **/
	...
	protected $_linear_grants_reference; 	// ARRAY : Droit par défaut linéarisé
...
echo "DEFAULT : "; print_r($this->_grants_result); //// droit obtenu via default
...
//echo "\n\nREWRITE GROUP : "; print_r($this->_grants_result); //// droit obtenu via rewrite par group
...
//echo "\n\nREWRITE USER : "; print_r($this->_grants_result); //// droit obtenu via rewrite par group
...
//echo "\n\nALIASING : "; print_r($this->_grants_result); //// droit obtenu via alias
		...
		echo "\n\nDEPENDENCIES TO FINAL : "; print_r($this->_grants_result); //exit();/// Droit finaux obtenu
	...
	/** ------------------------------------------------------------------------- **
	/** --- Méthode de transcription des droits en donnée de sessions en JSON --- **
	/** ------------------------------------------------------------------------- **/
	...
	private function grant_level_reader($level, $grant_backtrace, $forcibly_false=false){
		...
		if(!$forcibly_false){
			foreach($level as $key => $value){
				...
				if(array_key_exists($key, $this->_reserved_words) && $this->_reserved_words[$key] === 1){
		...
		foreach($level as $key => $value){
			...
			if(array_key_exists($key, $this->_reserved_words)){
				switch($this->_reserved_words[$key]){
		...
		// rewrite : User > Group > Default
		// Si rewrite check l'existance, sinon not exist skip, si exist, check si le grant_backtrace au plus haut si vrai, sinon untouch
	}
