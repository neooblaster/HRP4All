...
/** ---			-  Ajout d'une constante pour lister les caractères à échapper pour les textes client							---
/** ---																																						---
			...
			if($file !== ''){
				...
				if(file_exists($path)){
					...
					for($i = 0; $i < count($resources); $i++){
						...
						if($cst === 'true'){
							$client_side_target[] = Array("VAR_KEY" => $key, "VAR_VALUE" => addcslashes($value, self::ESCAPED_CHAR));
						}
